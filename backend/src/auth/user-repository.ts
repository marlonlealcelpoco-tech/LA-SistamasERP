...
export const ROLE_NAMES = ["ADMIN", "VENDAS", "ESTOQUE", "FINANCEIRO"] as const;
...
export class UserRepository {
  ...
  async createWithRoles(
    ...
    try {
      ...
      if (assigned.rows.length !== roles.length) {
        throw new Error("Um ou mais perfis não existem.");
      }
  ...
  async replaceRoles(userId: number, roles: UserRole[]): Promise<UserSummary | undefined> {
    ...
    try {
      ...
      const existing = await client.query<UserRecord>(
        "SELECT id, name, email, password_hash, active FROM users WHERE id = $1 FOR UPDATE",
        [userId]
      );
