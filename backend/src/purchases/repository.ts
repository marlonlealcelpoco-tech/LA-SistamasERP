...
export type ImportPreviewItem = XmlPurchaseItem & {
  match: ProductMatch | null;
  costBefore: number | null;
  salePriceBefore: number | null;
  suggestedSalePrice: number | null;
  resolution: "FOUND" | "UNRESOLVED";
};
...
export class PurchaseRepository {
  ...
  async previewXml(xml: XmlPurchase): Promise<ImportPreviewItem[]> {
    ...
    const result = await this.pool.query<ProductMatch>(
      `SELECT id, code, name, cost, sale_price, profit_margin_pct
       FROM products WHERE UPPER(code) = ANY($1::text[])`,
      [codes]
    );
    ...
    return xml.items.map((item) => {
      ...
      return {
        ...item,
        match,
        costBefore: match ? Number(match.cost) : null,
        salePriceBefore: match ? Number(match.sale_price) : null,
        suggestedSalePrice: match ? Number((newCost * (1 + margin / 100)).toFixed(2)) : null,
        resolution: match ? "FOUND" : "UNRESOLVED"
      };
    });
  }

  async createManual(supplierId: number, items: PurchaseItemInput[]): Promise<CreatedPurchase> {
  ...
  async confirm(
    purchaseId: number,
    salePriceUpdates: Map<number, boolean>
  ): Promise<{ kind: "not_found" | "already_confirmed" } | { kind: "success"; purchase: PurchaseRecord }> {
    const client = await this.pool.connect();

    try {
      await client.query("BEGIN");
      const purchase = await client.query<PurchaseRecord>(
        `SELECT id, supplier_id, status, total, source, xml_access_key, created_at
      ...
      const items = await client.query<PurchaseItemRecord>(
        `SELECT id, product_id, quantity, unit_cost, total, xml_item_number, source_code, source_description,
      ...
      if (items.rows.some((item) => item.product_id === null)) {
        throw new Error("Todos os itens devem estar vinculados a um produto antes da confirmação.");
      }
      ...
      for (const item of items.rows) {
        ...
        await client.query(
          "UPDATE products SET cost = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1",
          [productId, item.unit_cost]
        );

        if (salePriceUpdates.get(item.id) && item.suggested_sale_price !== null) {
          await client.query(
            "UPDATE products SET sale_price = $2, updated_at = CURRENT_TIMESTAMP WHERE id = $1",
            [productId, item.suggested_sale_price]
          );
        }

        const stock = await client.query<{ quantity: string }>(
          "UPDATE stock SET quantity = quantity + $2 WHERE product_id = $1 RETURNING quantity",
        ...
        await client.query(
          ...
          [productId, item.quantity, `COMPRA-${purchaseId}`, "Entrada por confirmação de compra"]
        );
      }

      const confirmed = await client.query<PurchaseRecord>(
        `UPDATE purchases SET status = 'CONFIRMED' WHERE id = $1
         RETURNING id, supplier_id, status, total, source, xml_access_key, created_at`,
        [purchaseId]
      );
  ...
  private async createPurchase(input: {
    supplierId: number;
    source: "MANUAL" | "XML";
    items: PurchaseItemInput[];
    ...
    try {
      ...
      const purchase = await client.query<PurchaseRecord>(
        `INSERT INTO purchases (supplier_id, status, total, source, xml_access_key, xml_raw)
         VALUES ($1, 'DRAFT', $2, $3, $4, $5)
         RETURNING id, supplier_id, status, total, source, xml_access_key, created_at`,
        [input.supplierId, total, input.source, input.xmlAccessKey, input.xmlRaw]
      );
      const items: PurchaseItemRecord[] = [];

      for (const item of input.items) {
        items.push(await this.insertItem(client, purchase.rows[0].id, item));
      }
  ...
  private async insertItem(client: PoolClient, purchaseId: number, item: PurchaseItemInput): Promise<PurchaseItemRecord> {
    const product = await client.query<ProductMatch>(
      `SELECT id, code, name, cost, sale_price, profit_margin_pct
