...
async function request<T>(path:string,init:RequestInit={}){const headers=new Headers(init.headers);headers.set("Content-Type","application/json");if(token)headers.set("Authorization",`Bearer ${token}`);const r=await fetch(`${API_URL}${path}`,{...init,headers});const body=await r.json().catch(()=>({}));if(!r.ok)throw new Error(body.message||"Falha na comunicação com o servidor");return body as T}
export async function login(email:string,password:string){const data=await request<{token:string}>("/auth/login",{method:"POST",body:JSON.stringify({email,password})});setToken(data.token);return data}
...
export const api={
 ...
 productCreate:(p:Omit<Product,"id"|"category">)=>request<{product:Product}>("/quotations/products",{method:"POST",body:JSON.stringify(p)}),
 productUpdate:(id:number,p:Omit<Product,"id"|"category">)=>request<{product:Product}>(`/quotations/products/${id}`,{method:"PUT",body:JSON.stringify(p)}),
 ...
 quotationCreate:(q:{productId:number;supplierId:number;priceType:"un"|"cx";value:number})=>request("/quotations",{method:"POST",body:JSON.stringify(q)}),
